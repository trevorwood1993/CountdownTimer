...
function checkHoliday(d,hour,first,daylightSavingsOn){
	...
	if(staticHolidays.indexOf(shc) >= 0 || holidays.indexOf(hc) >= 0){
		...
		return 24+checkHoliday(d,hour,0,daylightSavingsOn);
	}
	// console.log("Not a holiday: "+hc);
	return skipSunday(d,hour,first,daylightSavingsOn);
	
}
function skipSunday(d,hour,first,daylightSavingsOn){
	var d = new Date(d),
			d_hours = d.getHours(),
			d_day = d.getDay();

	var daylightSavingsSender = 0;
	d = new Date(d.setDate(d.getDate() + 1));	
	if(d.dst() && daylightSavingsOn == 0){
		daylightSavingsSender = 1;
	}
	...
	if(first == 1){
		...
		if(d_day == 6 && d_hours >= hour || d_day == 0 && d_hours <= hour){
			...
			return 24+daylightSavingsSender;
		}
	}else{
	...
	return 0+daylightSavingsSender;
}
function countDownTimer(){
	...
	if(z_hours < 0){
		...
		d = d.setDate(d.getDate() + 1);	
		z_hours = 24+z_hours+checkHoliday(d,hourToCountTo,0,daylightSavingsOn);
	}else{
		z_hours = z_hours+checkHoliday(d,hourToCountTo,1,daylightSavingsOn);
	}
